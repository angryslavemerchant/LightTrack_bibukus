"""
tracking/distill_xattnRemote_freshhead.py

Same as distill_xattnRemote.py but with the cls tower reinitialised from scratch.
Both xattn and the cls tower co-adapt from random init, removing the conflict
between pretrained tower expectations and xattn's different feature distribution.

Frozen: backbone + neck.
Trainable (all random init): feature_fusor (xattn pw_corr + adj_layer) + head.
"""
...
def train():
    ...
    # --- student: pretrained backbone+neck, everything else random ---
    ...
    # reinitialise adj_layer and head so they co-adapt with xattn from scratch
    reinit(student.feature_fusor.adj_layer)
    reinit(student.head)
    ...
    for epoch in epoch_bar:
        ...
        ckpt_path = os.path.join(SAVE_DIR, f"xattn_freshhead_epoch{epoch:03d}.pth")
